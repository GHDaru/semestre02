...
with r:
    ...
    cat = final[final.desc == messy.loc[st.session_state.count, 'nm_item']].nm_product.tolist()[0]
...
with l:
    ...
    st.table(final.loc[final.desc == messy.loc[st.session_state.count, 'nm_item'], ['Result','nm_product','Ratio']]) 
with m:
...
with r:
    ...
    st.table(df.loc[df.gtin.isin(df.loc[df.nm_item == messy.loc[st.session_state.count, 'nm_item']].gtin.tolist()), ['id_item','nm_item','gtin','nm_product']])
